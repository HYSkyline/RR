...
def main():			# 程序主体函数
	...
	init_id = input('样本起点:'.decode('utf-8').encode('gbk'))			# 输入样本起点的ID
	ranger = input('样本数:'.decode('utf-8').encode('gbk'))			# 输入需要提取的样本数量
	final_id = init_id + ranger			# 计算样本终点的ID
	try:			# 如果程序意外中断,则可以直接重新运行程序完成剩余样本的提取
		f = open(r"rr_rd" + date_now + ".txt", 'r')			# 打开程序中断时的文件,如果首次运行则出现IOError输入输出错误
		contents_exist = f.readlines()			# 读取文件内容
		f.close()			# 关闭文件
		init_id = init_id + len(contents_exist)			# 重新计算样本起点,定位至程序中断时的最后一个目标的ID
		ranger = ranger - len(contents_exist)			# 重新计算需要补全的样本数量
	except IOError:			# 程序首次运行
		pass			# 无操作
	...
	thread_init(randompage_login, random_urllist, rr_opener, threadnum)			# 构建多线程模型,并开始提取

	t1 = time.time()			# 计算程序结束运行的时间
	f = open(r"rr_rd_error" + date_now + ".txt", 'r')			# 打开提取错误列表
	error_list = f.readlines()			# 读取错误列表内容
	f.close()			# 关闭错误列表文件
	content = "(" + str(len(random_urllist) - len(error_list)) + "/" + str(len(random_urllist)) + ")"			# 输出提取完成数量\样本总量
	link_report(t0, t1, content)			# 输出程序报告
	fpath = []			# 创建文件路径列表,用以邮件的附件准备
	fpath.append(r"rr_rd" + date_now + ".txt")			# 导入提取输出文件
	fpath.append(r"rr_rd_error" + date_now + ".txt")			# 导入错误列表文件
...
def random_urllist_search(init_id, final_id):			# 构建目标基本资料网页URL列表
	targets = []			# 准备目标基本资料网页URL列表
	for i in range(init_id, final_id):			# 对样本范围进行遍历
		target = 'http://3g.renren.com/details.do?id=' + str(i) + '&htf=738&sid=9s2rB_0HVrp1WYcJGhQ6AM&qg5ups'			# 构建目标基本资料网页URL
		targets.append(target)			# 添加至列表中
	# print u"第" + str(i+1) + u"页目标网页地址采集完毕"
	print u"已确认" + str(len(targets)) + u"个目标"			# 报告URL列表准备完成
	print "--" * 10			# 分割线
	return targets			# 返回目标基本资料页URL列表


def randompage_login(opener, urls, code):			# 对给定列表内所有URL进行数据提取,并写入文件
	error_target = []			# 准备错误列表
	for each in urls:			# 对给定列表内所有URL进行遍历
		...
		try:
			...
			rd_fpath = r"rr_rd" + time.strftime("%y%m%d", time.localtime()) + ".txt"			# 确定数据输出文件路径
			time_write(rd_fpath, content)			# 写入内容
		except Exception:			# 基本资料页打开超时(浏览器打开时显示 系统繁忙,可能为目标用户不存在或已经注销)
			error_target.append("用户不存在或其他错误\t" + each)			# 将错误添加至错误列表中
	error_fpath = r"rr_rd_error" + time.strftime("%y%m%d", time.localtime()) + ".txt"			# 确定错误立标文件路径
	error_content = "\n".join(error_target) + "\n"			# 构建错误内容
	if error_content != "\n":			# 错误列表不为空,即有错误
